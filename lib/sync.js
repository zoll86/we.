...
// ─── Presence + last_seen (v0.8) ──────────────────────────────────────

export async function updateLastSeen(pairId, memberId) {
  if (!supabase) return;
  // a teljes last_seen objektumot beolvassuk és frissítjük (jsonb partial-update bonyolult)
  const { data: cur } = await supabase
    .from('pairs')
    .select('last_seen')
    .eq('id', pairId)
    .maybeSingle();
  const lastSeen = cur?.last_seen || {};
  lastSeen[memberId] = new Date().toISOString();
  const { error } = await supabase
    .from('pairs')
    .update({ last_seen: lastSeen })
    .eq('id', pairId);
  if (error) console.error('[sync] updateLastSeen hiba:', error);
}

let presenceChannel = null;
let presenceHeartbeat = null;

export function joinPresence(pairId, memberId, callbacks = {}) {
  if (!supabase) return;
  if (presenceChannel) {
  ...
  presenceChannel = supabase.channel(`presence:${pairId}`, {
    config: { presence: { key: memberId } },
  });
  const fireSync = () => {
    if (!presenceChannel) return;
    const stateMap = presenceChannel.presenceState();
    const onlineMembers = Object.keys(stateMap);
    callbacks.onPresenceSync?.(onlineMembers);
  };
  presenceChannel
    ...
    .subscribe(async status => {
      if (status === 'SUBSCRIBED') {
        ...
        // 30 mp-enként újra-track (hogy a szervernél is friss maradjon)
        ...
        // azonnal sync-eljük a state-et hogy a saját jelenlét is látsszon
        setTimeout(fireSync, 200);
      }
    });
}
...
// ─── Csillám-buborék üzenetek (v0.9) ──────────────────────────────────

export async function createCsillamMessage(pairId, authorId, type, payload, deliveryAt = null, expiresAt = null) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from('csillam_messages')
    .insert({
      pair_id: pairId,
      author_id: authorId,
      type,
      payload,
      delivery_at: (deliveryAt || new Date()).toISOString(),
      expires_at: expiresAt ? expiresAt.toISOString() : null,
    })
    .select()
    .single();
  if (error) console.error('[sync] createCsillamMessage hiba:', error);
  return data;
}

export async function loadActiveCsillamMessage(pairId) {
  // a legfrissebb olyan, aminek delivery_at <= most ÉS expires_at > most (vagy NULL)
  if (!supabase) return null;
  ...
  if (error) {
    console.error('[sync] loadActiveCsillamMessage hiba:', error);
    return null;
  }
  if (!data || data.length === 0) return null;
  const msg = data[0];
  // expires_at ellenőrzés
  if (msg.expires_at && new Date(msg.expires_at) <= new Date()) return null;
  return msg;
}

export async function loadPendingCsillamMessages(pairId) {
  // jövőbeli kézbesítéseket is letöltjük, hogy a kliens időzíteni tudja a megjelenést
  if (!supabase) return [];
  const { data } = await supabase
    .from('csillam_messages')
    .select('*')
    .eq('pair_id', pairId)
    .gte('delivery_at', new Date().toISOString())
    .order('delivery_at', { ascending: true })
    .limit(20);
  return data || [];
}

// v0.11: teljes buborék-történet (folyam-megjelenítéshez)
export async function loadCsillamHistory(pairId, limit = 50) {
  if (!supabase) return [];
  const { data } = await supabase
    .from('csillam_messages')
    .select('*')
    ...
    .order('delivery_at', { ascending: false })
    .limit(limit);
  return (data || []).reverse(); // legrégibb elöl, legfrissebb a végén
}
...
// ─── Vágyak / jegyzetek (közös bakancslista, v0.10-ben kibővítve) ─────

export async function addWish(pairId, memberId, text, opts = {}) {
  // opts: { note, category, target_date, time_tag }
  if (!supabase) return null;
  const payload = {
    pair_id: pairId,
    text,
  ...
  if (opts.target_date) payload.target_date = opts.target_date;
  if (opts.time_tag) payload.time_tag = opts.time_tag;
  const { data, error } = await supabase
    .from('vagyak')
    .insert(payload)
    .select()
...
export async function markVagySurfaced(vagyId) {
  if (!supabase) return;
  const { error } = await supabase
    .from('vagyak')
    .update({ last_surfaced_at: new Date().toISOString() })
    .eq('id', vagyId);
  if (error) console.error('[sync] markVagySurfaced hiba:', error);
}
...
export function subscribeToPair(pairId, callbacks) {
  ...
  pairChannel = supabase
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'csillam_messages', filter: `pair_id=eq.${pairId}` },
      payload => {
        callbacks.onCsillamMessage?.(payload);
      }
    )
    .subscribe(status => {
      if (status === 'SUBSCRIBED') {
